...
export default function UpdateEmp() {
    ...
    useEffect(() => {
        if (state) {
            ...
            // Fetch employee data from Firestore if state is missing
            CRUD_OP.getEmployee(id).then((doc) => {
                if (doc.exists()) {
                    const data = doc.data()
                    setName(data.name || "")
                    setEmail(data.email || "")
                    ...
                    setMessage({ error: true, msg: "Employee not found." })
                }
            }).catch((err) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setMessage("")
        try {
            await CRUD_OP.updateEmployee(id, updateEmp)
    ...
    return (
        <div className="updateEmp">
            <Navbar />
            {message?.msg && (
                <div className="alert-container container-fluid d-flex justify-content-between align-items-center"
                    role='alert'
                    style={{
                        fontSize: "1.4rem",
                ...
                >
                    <p className={message?.error ? "text-danger" : "text-success"}>{message.msg}</p>
                    <div className="btn-cont p-2 mx-2">
